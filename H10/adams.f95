	...
	subroutine interadams(X0,t0,Xnew,X, h, predictions)
	! Процедура находит вектор Xnew(t0+h) из вектора X0(t0)
	implicit none
	real(mp), dimension(1:), intent(in) :: X0
	real(mp), intent(in) :: t0, h
	real(mp), dimension(1:size(X0)), intent(out) :: Xnew, predictions
	real(mp), dimension(-Ninteradams+2:0,1:size(X0)) :: X
	real(mp), dimension(1:size(X0)) :: r
	integer :: j, n
	...
	call newton(predictions,10000,Xnew,ft)


	contains

	function ft(t, X) result(ff)
